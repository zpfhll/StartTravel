...
class ModifyTravelActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setTitle(
            getString(R.string.modify_travel_001),
            true,
        ...
        travel_money_editText.isEnabled = false
    ...
    fun onGetMessage(message: EventBusMessage) {
        if (message.message == MODIFY_TRAVEL){
            travel = message.travel?.getTravelData()?.value
            travelType = when(travel?.type ?: 0) {
                0 -> TravelTypeEnum.MONEY_TRAVEL
                1 -> TravelTypeEnum.FREE_TRAVEL
                else -> TravelTypeEnum.MONEY_TRAVEL
            }
            when(travelType){
            ...
            travel?.name?.let {
                travel_name_editText.setText(it)
            }

            travel?.memo?.let {
                travel_memo_editText.setText(it)
            }
